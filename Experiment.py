...
class Experiment:
    ...
    def prepare_models(self, **kwargs):
        ...
        if self.network["AR"] == "LSTM":
            self.model_AR = LSTM(context_len, self.pred_horizon)
        elif self.network["AR"] == "KAN":
            self.model_AR = KAN(context_len, self.pred_horizon, [context_len*2+1])
        elif self.network["AR"] == "GRU":
            self.model_AR = GRU(context_len, self.pred_horizon)
        elif self.network["AR"] == "Elman":
            self.model_AR = Elman(context_len, self.pred_horizon)
        else:
            raise ValueError(f"{self.network['AR']} is not implemented for AR model.")
        
        if self.network["MA"] == "LSTM":
            self.model_MA = LSTM(self.errors_context, self.pred_horizon)
        elif self.network["MA"] == "KAN":
            self.model_MA = KAN(self.errors_context, self.pred_horizon, [self.errors_context*2+1])
        elif self.network["MA"] == "GRU":
            self.model_MA = GRU(self.errors_context, self.pred_horizon)
        elif self.network["MA"] == "Elman":
            self.model_MA = Elman(self.errors_context, self.pred_horizon)
        else:
